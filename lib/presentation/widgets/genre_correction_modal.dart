...
void showGenreCorrectionModal(BuildContext context, Track track) {
  ...
  final List<String> allCanonicalGenres =
      graph.knownGenres.toList()..sort();
              ...
              children: [
                  ...
                  children: [
                    Expanded(
                      child: GestureDetector(
                        onTap: () =>
                            setModalState(() => targetType = 'track'),
                        child: Container(
                          padding:
                    ...
                    Expanded(
                      child: GestureDetector(
                        onTap: () =>
                            setModalState(() => targetType = 'artist'),
                        child: Container(
                          padding:
                              const EdgeInsets.symmetric(vertical: 10),
                          decoration: BoxDecoration(
                ...
                if (selectedGenres.isNotEmpty) ...[
                  ...
                  Wrap(
                    spacing: 6,
                    runSpacing: 6,
                    children: selectedGenres.map((g) => InputChip(
                      label: Text(
                        g,
                        style: const TextStyle(
                          color: Colors.black,
                      ...
                      onDeleted: () =>
                          setModalState(() => selectedGenres.remove(g)),
                    )).toList(),
                  ),
                ],
                const SizedBox(height: 16),
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: selectedGenres.isEmpty
                        ...
                        : () async {
                            ...
                            if (targetType == 'track') {
                              success =
                                  await service.submitTrackSuggestion(
                                track: track,
                              ...
                              success =
                                  await service.submitArtistSuggestion(
                                artistName:
                                    track.author ?? 'Unknown Artist',
                                genres:
                                    List<String>.from(selectedGenres),
                                country: track.country,
                              );
                            }
                            if (ctx.mounted) {
