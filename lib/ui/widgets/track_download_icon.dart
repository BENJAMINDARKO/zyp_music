...
///    `box.containsKey(trackId)` semantic.
...
/// - idle    -> not in Hive, no active stream -> download outline icon
/// - caching -> transient `_activeCaching` set -> CircularProgressIndicator
/// - success -> in Hive AND `File.exists()` -> static checkmark
///
...
class _TrackDownloadIconState extends State<TrackDownloadIcon> {
  ...
  void _onTap() {
    ...
    // Spec §1: explicit Hive check before triggering the download pipeline.
    if (cache.isCached(widget.track.id)) {
      return;
    }
